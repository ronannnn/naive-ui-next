...
import { RDnd, RPopselectButton } from '~/components'
import { cloneJson } from '~/composables'
  ...
  if (foundOption) {
    ...
    if (!foundOption.icon) {
      switch (foundOption.type) {
        case 'select':
          iconVNode = () => h('div', { class: 'i-tabler-selector' })
          break
        case 'input':
          iconVNode = () => h('div', { class: 'i-tabler-edit' })
          break
        case 'date':
        case 'dateRange':
          iconVNode = () => h('div', { class: 'i-tabler-calendar-month' })
          break
      }
    }
